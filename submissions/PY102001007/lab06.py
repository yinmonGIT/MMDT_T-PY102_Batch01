...
def schedule_next_job(jobs, new_job):
    jobs.append(new_job)
    last_idx = len(jobs) - 1
    jobs = helper_fun1_(jobs, last_idx)
    return jobs
...
def personal_priority_q():
    ...
    for idx in range(1, len(priorty_q)):
        priorty_q = helper_fun1_(priorty_q, idx)
 
    new_item = (6, "security")
    priorty_q.append(new_item)
 
    last = len(priorty_q) - 1
    priorty_q = helper_fun1_(priorty_q, last)
